...
def merge_sort(dataset):
  ...
  if len(dataset) <= 1:
    ...
    datasubset_1 = dataset[0:(len(dataset)/2)] #start to half
    datasubset_2 = dataset[(len(dataset)/2):] #half to end
    dataset = merge_sort(datasubset_1).append(merge_sort(datasubset_2))
