...
psuedo code
    ...
    for i < len(list[i]) in list[i]:

        <!-- current = list[i]
        next = list[i++] -->

        if list[i] > list[i++]:
            global smaller and larger?
            smaller = list[i++]
            larger = list[i]
...
2. For all indices EXCEPT the last index:
   ...
   assume current number is smallest, smallest = arr[i]
   while j in range(i+1, len(arr)-2):
     
    if the smallest <= the next_number/ arr[j]
   j +=1
   else:
   smallest = arr[j]
   continue
